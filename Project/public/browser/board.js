...
Board.prototype.updateAlgorithmInfoPanel = function(name, type, guaranteesShortest, description) {
  const title = document.getElementById("algorithmInfoTitle");
  const body = document.getElementById("algorithmInfoBody");
  if (!title || !body) return;
  const guaranteeText = guaranteesShortest ? "guarantees" : "does not guarantee";
  title.textContent = name;
  body.textContent = `${description} This algorithm is ${type} and ${guaranteeText} the shortest path.`;
};
...
Board.prototype.changeStartNodeImages = function() {
  ...
  let description = "Explore the grid to see how this algorithm searches for a path.";
  if (this.currentAlgorithm === "bfs") {
    description = "Breadth-first Search explores in layers from the start node.";
  } else if (this.currentAlgorithm === "dfs") {
    description = "Depth-first Search follows one path deeply before backtracking.";
  } else if (this.currentAlgorithm === "dijkstra") {
    description = "Dijkstra's Algorithm expands the cheapest known frontier first.";
  } else if (this.currentAlgorithm === "astar") {
    description = "A* Search combines path cost and heuristic distance to the target.";
  } else if (this.currentAlgorithm === "greedy") {
    description = "Greedy Best-first Search strongly prefers nodes that look closest to the target.";
  } else if (this.currentAlgorithm === "CLA" && this.currentHeuristic !== "extraPoweredManhattanDistance") {
    description = "Swarm Algorithm blends path cost with a strong directional heuristic.";
  } else if (this.currentAlgorithm === "CLA" && this.currentHeuristic === "extraPoweredManhattanDistance") {
    description = "Convergent Swarm uses an even stronger heuristic to drive toward the target faster.";
  } else if (this.currentAlgorithm === "bidirectional") {
    description = "Bidirectional Swarm searches from both start and target until the frontiers meet.";
  }

  const type = unweighted.includes(this.currentAlgorithm) ? "unweighted" : "weighted";
  const guaranteesShortest = this.currentAlgorithm === "bfs" || guaranteed.includes(this.currentAlgorithm);
  this.updateAlgorithmInfoPanel(name, type, guaranteesShortest, description);
};
